...
class TestBuildReferences:
    def test_dedupe_keep_highest(self):
        rows = [
            {"document": "A.pdf", "similarity": 0.9, "document_id": "d1", "dataset_id": "ds"},
            {"document": "A.pdf", "similarity": 0.7, "document_id": "d1", "dataset_id": "ds"},
            {"document": "B.docx", "similarity": 0.8, "document_id": "d2", "dataset_id": "ds"},
        ]
...
class TestDedupeCitations:
    def test_cross_call_dedupe(self):
        # 一次 run 内两次 knowledge_search 命中同一文档 → 去重取最高相似度
        citations = [
            {"document": "A", "similarity": 0.7, "url": "u1"},
            {"document": "A", "similarity": 0.9, "url": "u1"},
            {"document": "B", "similarity": 0.8, "url": "u2"},
        ]
