...
- 素材原因必须标来源，格式「Btype·【标题】」——标题取该条素材 content 开头【】内的文字，如「故障记录·【1#变桨速度超限】」「场站日志（集控）·【早会内容】」；content 无【】时只写 Btype。来源即数据证实依据，推测项不标来源只标「（推测）」。
...
**二、[核心指标2]** 同上。多指标多分节。
**三、异常关注与归因** 偏差最大/排名突出的实体逐条点名（场站/项目），给指标+变化+同比环比对比+归因——每条归因后附来源「Btype·【标题】」，推测的标「（推测）」。
